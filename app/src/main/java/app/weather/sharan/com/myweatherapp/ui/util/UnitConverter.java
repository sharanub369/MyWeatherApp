...
public class UnitConverter {
    ...
    public static float convertTemperature(float temperature, SharedPreferences sp) {
        if (sp.getString(Constants.KEY_TEMP_UNIT, "C").equals("C")) {
            return UnitConverter.kelvinToCelsius(temperature);
        } else if (sp.getString(Constants.KEY_TEMP_UNIT, "C").equals("F")) {
    ...
    public static String getRainString(double rain, SharedPreferences sp) {
        if (rain > 0) {
            ...
            } else if (sp.getString("lengthUnit", "mm").equals("in")) {
                rain = rain / 25.4;
                if (rain < 0.01) {
                    return " (<0.01 in)";
                } else {
    ...
    public static double convertRain(double rain, SharedPreferences sp) {
        double rainVal = 0;
        if (rain > 0) {
            if (sp.getString("lengthUnit", "mm").equals("mm")) {
                if (rain < 0.1) {
            ...
            } else if (sp.getString("lengthUnit", "mm").equals("cm")) {
                rain = rain / 10;
                if (rain < 0.01) {
                    rainVal = 0.01;
                } else {
